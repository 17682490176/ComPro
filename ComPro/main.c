...
static void log_line(const WCHAR *s, COLORREF c) {
    ...
    /* force unified font+charset at cursor to prevent RichEdit auto font switching */
    ...
    if(g_dispLen>60000){SendMessageW(g_hDisp,EM_SETSEL,0,g_dispLen-50000);SendMessageW(g_hDisp,EM_REPLACESEL,FALSE,(LPARAM)L"");g_dispLen=GetWindowTextLengthW(g_hDisp);}
    SCROLLINFO si={sizeof(si),SIF_ALL};GetScrollInfo(g_hDisp,SB_VERT,&si);
    if(si.nPos+(int)si.nPage>=si.nMax-30)SendMessageW(g_hDisp,WM_VSCROLL,SB_BOTTOM,0);
}
...
static int is_consumed(int idx) {
    for (int j = idx - 1; j >= 0 && g_reg[j].addr <= g_reg[idx].addr; j--) {
        ...
        if (g_reg[j].addr + nr > g_reg[idx].addr) {
            /* if j itself is consumed, skip — it doesn't own this range */
...
static void do_send(BOOL silent) {
    ...
    if (tcp) {
        int sent = send(g_sock, (char*)sb, sl, 0);
...
static DWORD WINAPI modbus_scan_worker(LPVOID p) {
    ...
    for (;;) {
        ...
        if (connected) {
            ...
            if (send(sock, (char*)a->sendBuf, a->sendLen, 0) == a->sendLen) {
                BYTE rsp[256];
...
static LRESULT CALLBACK bited_proc(HWND h, UINT m, WPARAM w, LPARAM l) {
    ...
    case WM_PAINT: {
        ...
        HFONT bf = CreateFontW(14, 0, 0, 0, FW_BOLD, 0, 0, 0, DEFAULT_CHARSET,
        ...
        DeleteObject(bf); DeleteObject(sf);
        EndPaint(h, &ps);
        return 0;
    }
    case WM_LBUTTONDOWN: {
...
static DWORD WINAPI tcp_rd_thread(LPVOID a) {
    ...
    while (g_run) {
        ...
        if (s == INVALID_SOCKET) {
            ...
            if (s == INVALID_SOCKET) { for (int _w = 0; _w < 10 && g_run; _w++) Sleep(100);
            continue; }
            ...
            { fd_set wfds; struct timeval tv = {0, 100000}; int ok = 0;
              ...
              if (!ok || !g_run) { closesocket(s); s = INVALID_SOCKET;
                  for (int _w = 0; _w < 10 && g_run; _w++) Sleep(100);
                  continue; }
            }
...
static LRESULT CALLBACK about_proc(HWND h, UINT m, WPARAM w, LPARAM l) {
    if (m == WM_PAINT) {
        ...
        DrawTextW(dc, L"ComPro — Modbus RTU/TCP 调试工具", -1, &tr, DT_LEFT | DT_VCENTER);
        ...
        DrawTextW(dc, L"版本 3.1 | 纯 C Win32 + GCC/MinGW-w64 | 2026", -1, &vr, DT_LEFT | DT_VCENTER);
        SelectObject(dc, oldF);
